 ...
 */
class JBPJWTAuthenticationExtension extends Extension
{
    /**
     * {@inheritdoc}
     */
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        $container->setParameter('jbp_jwt_authentication.private_key_path', $config['private_key_path']);
        $container->setParameter('jbp_jwt_authentication.public_key_path', $config['public_key_path']);
        $container->setParameter('jbp_jwt_authentication.pass_phrase', $config['pass_phrase']);
        $container->setParameter('jbp_jwt_authentication.token_ttl', $config['token_ttl']);
        $container->setParameter('jbp_jwt_authentication.refresh_ttl', $config['refresh_ttl']);
        $container->setParameter('jbp_jwt_authentication.user_identity_field', $config['user_identity_field']);
        $encoderConfig = $config['encoder'];
        $container->setAlias('jbp_jwt_authentication.encoder', $encoderConfig['service']);
        $container->setAlias(JWTEncoderInterface::class, 'jbp_jwt_authentication.encoder');
        $container->setAlias(
            'jbp_jwt_authentication.key_loader',
            'jbp_jwt_authentication.key_loader.'.('openssl' === $encoderConfig['crypto_engine'] ? $encoderConfig['crypto_engine'] : 'raw')
        );

        $container->setParameter('jbp_jwt_authentication.encoder.signature_algorithm', $encoderConfig['signature_algorithm']);
        $container->setParameter('jbp_jwt_authentication.encoder.crypto_engine', $encoderConfig['crypto_engine']);

        $container
            ->getDefinition('jbp_jwt_authentication.extractor.chain_extractor')
    ...
    {
        $container
            ->findDefinition('jbp_jwt_authentication.encoder.default')
            ->addAutowiringType(JWTEncoderInterface::class);

        $container
            ->getDefinition('jbp_jwt_authentication.jws_provider.default')
        ...
        $container
            ->getDefinition('jbp_jwt_authentication.jwt_manager')
    ...
    private static function createTokenExtractors(ContainerBuilder $container, array $tokenExtractorsConfig)
    {
        ...
        if ($tokenExtractorsConfig['authorization_header']['enabled']) {
            $authorizationHeaderExtractorId = 'jbp_jwt_authentication.extractor.authorization_header_extractor';
            $container
        ...
        if ($tokenExtractorsConfig['query_parameter']['enabled']) {
            $queryParameterExtractorId = 'jbp_jwt_authentication.extractor.query_parameter_extractor';
            $container
        ...
        if ($tokenExtractorsConfig['cookie']['enabled']) {
            $cookieExtractorId = 'jbp_jwt_authentication.extractor.cookie_extractor';
            $container
