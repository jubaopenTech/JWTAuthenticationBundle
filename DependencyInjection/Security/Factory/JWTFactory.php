...
class JWTFactory implements SecurityFactoryInterface
{
    ...
    public function create(ContainerBuilder $container, $id, $config, $userProvider, $defaultEntryPoint)
    {
        ...
        if ($config['authorization_header']['enabled']) {
            $authorizationHeaderExtractorId = 'jbp_jwt_authentication.extractor.authorization_header_extractor.'.$id;
            $container
                ->setDefinition($authorizationHeaderExtractorId, $this->createChildDefinition('jbp_jwt_authentication.extractor.authorization_header_extractor'))
        ...
        if ($config['query_parameter']['enabled']) {
            $queryParameterExtractorId = 'jbp_jwt_authentication.extractor.query_parameter_extractor.'.$id;
            $container
                ->setDefinition($queryParameterExtractorId, $this->createChildDefinition('jbp_jwt_authentication.extractor.query_parameter_extractor'))
        ...
        if ($config['cookie']['enabled']) {
            $cookieExtractorId = 'jbp_jwt_authentication.extractor.cookie_extractor.'.$id;
            $container
                ->setDefinition($cookieExtractorId, $this->createChildDefinition('jbp_jwt_authentication.extractor.cookie_extractor'))
        ...
        $node
            ->children()
                ...
                ->scalarNode('authentication_provider')
                    ->defaultValue('jbp_jwt_authentication.security.authentication.provider')
                ->end()
                ->scalarNode('authentication_listener')
                    ->defaultValue('jbp_jwt_authentication.security.authentication.listener')
                ->end()
            ->end();
    }
    ...
    {
        $entryPointId = 'jbp_jwt_authentication.security.authentication.entry_point.'.$id;
        $container->setDefinition($entryPointId, $this->createChildDefinition('jbp_jwt_authentication.security.authentication.entry_point'));
