    ...
    protected function getKeyPath($type)
    {
        ...
        if (!is_file($path) || !is_readable($path)) {
            throw new \RuntimeException(
                sprintf('%s key "%s" does not exist or is not readable. Did you correctly set the "jbp_jwt_authentication.jwt_%s_key_path" config option?', ucfirst($type), $path, $type)
            );
        }
